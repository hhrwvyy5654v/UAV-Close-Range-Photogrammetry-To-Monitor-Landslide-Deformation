...
LastEditTime: 2023-06-12 09:28:05
...
for (i, j) in itertools.combinations(range(len(camera_poses)), 2):
    ...
    world_coord = triangulate_points(
        pixel_coords, pixel_coords, camera_matrix, rvec1, tvec1, rvec2, tvec2)[0]
    triangulated_points.append(world_coord)

# 对结果取平均值
world_coord_example = np.mean(triangulated_points, axis=0)
print(
    f"Pixel coordinate {pixel_coord_example} in the four images corresponds to world coordinate {world_coord_example}")
