...
file = './ArucoShot/IMG_20230420_153745.jpg'
npz = './Parameter/豪威OV48B.npz'
targetMarker = [0, 1]
groupnum = 4
...
while True:
    ...
    corners, ids, rejectedImgPoints = aruco.detectMarkers(
        img_gray, aruco.Dictionary_get(aruco.DICT_6X6_250), parameters=para)
    ...
    if corners:
        for i, marker_corners in zip(range(len(ids)), corners):  # 遍历探测到的marker ID
            cv2.polylines(
                frame, [marker_corners.astype(
                    int)], True, (0, 255, 255), 4, cv2.LINE_AA
            )
        ...
        # 计算每个marker中心的图像坐标
            ...
            id_coord[str(ids[i][0]) + '_' + str(i + 1)
                     ] = markerCenter  # 储存至一个字典里方便后面调取

        for i in targetMarker:
            res = {key: val for key, val in id_coord.items()
                   if key.startswith(str(i))}
            if len(res) == groupnum:  # 设定四个为一组

                center = sum(list(res.values())) / \
                    len(list(res.values()))  # 计算四个中心点的中心点
                image = cv2.circle(frame, (int(center[0]), int(
                    center[1])), radius=10, color=(0, 0, 255), thickness=-1)
                ...
                df.to_csv('../results/4in1.csv', mode='a',
                ...
                image = cv2.circle(frame, (int(avg_point[0]), int(
                    avg_point[1])), radius=10, color=(0, 0, 255), thickness=-1)
                vector = np.vectorize(np.int_)
                cv2.putText(
                    frame,
